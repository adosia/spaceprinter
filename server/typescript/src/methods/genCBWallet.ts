...
const genCBWallet: GenCBWallet =  ( jwToken, walletName, seedPhrase, walletPassPhrase, walletType ) => {
  return new Promise( async (resolve, reject) => {
    ...
    if( walletType == "printer" ) return await resolve(genPrinterWallet( db, walletName, seedPhrase, walletPassPhrase, walletType ));
    if( walletType == "general" ) return await resolve(createWallet( db, network, walletName, seedPhrase, walletPassPhrase, walletType));
  });
};
